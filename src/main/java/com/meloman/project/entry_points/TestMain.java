...
public class TestMain {
    public static void main(String[] args){
        ...
        // Test cloning
        AlbumT clonedAlbumT = albumT1.clone();
        System.out.println("\nCloned Album:");
        clonedAlbumT.displayInfo();

        // Sorting Albums by Year (Comparable)
        List<AlbumT> albumTList = new ArrayList<>();
        albumTList.add(albumT1);
        albumTList.add(albumT2);

        albumTList.sort(Comparator.naturalOrder()); // Sort by year using Comparable
        System.out.println("\nSorted Albums by Year:");
        for (AlbumT albumT : albumTList) {
            System.out.println(albumT.getTitle() + " (" + albumT.getYear() + ")");
        }

        // Sorting Albums by Title (Comparator)
        albumTList.sort(new AlbumTitleComparator()); // Sort by title using Comparator
        System.out.println("\nSorted Albums by Title:");
        for (AlbumT albumT : albumTList) {
            System.out.println(albumT.getTitle());
        }

        // Test MediaOwner and Polymorphism
        MediaOwner mediaOwner = new ArtistT("A3", "Artist Three", Arrays.asList(albumT1, albumT2), "Real Name Three", Arrays.asList("Alias5"), Arrays.asList("Group3"), new HashSet<>());
